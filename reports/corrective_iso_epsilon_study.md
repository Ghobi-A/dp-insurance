...
A third audit pass, again before dispatch, corrected the randomisation unit:

* **The ΔD null exchanged recipe labels per example.** Recipe is assigned at the
  trained-model level, so examples inside a seed are not independently
  exchangeable — treating one randomisation as several hundred understates the
  null's spread and makes the p-value anti-conservative. The confirmatory test
  is now the exact paired sign-flip over target seeds, enumerated exhaustively;
  per-example exchange survives only as a labelled exploratory sensitivity
  analysis that cannot produce a verdict.
* **Three seeds cannot support that test.** `2^3 = 8` assignments floor the
  two-sided p at 0.25. The confirmatory experiment is frozen at eight seeds
  (42–49); the ladder stays at three (42, 43, 44). Recorded in Amendment 4.
...
inclusion schedule, number of shadows, the eight confirmatory target seeds
(42–49, identical between recipes), δ, accountant, threat model, attack
implementation and subgroup inferential statistic. The two recipes
...
per-seed one-person TPR resolution.
...
The null is **no recipe-dependent redistribution of subgroup leakage**, and the
randomisation unit is the **target seed** — recipe is applied when a target
model and its matched shadow ensemble are trained, so every example score within
a seed carries one recipe assignment, not hundreds. The confirmatory null
therefore exchanges each seed's complete Recipe A and Recipe B score vectors
together, which negates that seed's contrast exactly:

```
T_obs = mean_s(ΔD_s),   null:  ΔD_s* = z_s · ΔD_s,  z_s ∈ {−1, +1}
p     = count(|T*| ≥ |T_obs|) / 2^S
```

All `2^S` sign assignments are enumerated exhaustively — 256 at the eight
confirmatory seeds — so the p-value is exact with no Monte Carlo error.
Reported: ΔD by seed, mean ΔD, the complete null distribution, the exact
p-value, the number of assignments and `resolution_by_seed`.

Per-example exchange is retained only as a labelled exploratory sensitivity
analysis. It is anti-conservative here, it exposes no key named `p_value`, and
`classify_redistribution` refuses to issue a verdict from it.
...
The confirmatory experiment runs on **eight** frozen target seeds — 42–49 —
identical between the two recipes. Three seeds would give only `2^3 = 8` sign
assignments, whose smallest attainable two-sided p is 0.25: the test could never
reject at α = 0.05. Eight seeds give 256 assignments and a floor of ≈ 0.0078.
**The ladder is unchanged at seeds 42, 43, 44**; its sole job is selecting ε
from aggregate attack power.

The order of operations is fixed: recipes frozen before the ladder → 3-seed
ladder selects ε by the aggregate rule alone → both recipes calibrated
independently to that ε (achieved ε within 0.05) → both run on the eight
confirmatory seeds → one exact paired seed-level sign-flip ΔD test.
